...
for d in d_array:
    if moleculetype == 'lambda':
        df = pd.read_csv(f'H_Rindler{d}d_lambda_clean.csv', names=[
                         'rho', 'H', 'b'], header=None)
        df['rho'] = df['rho'].round(1)
    elif moleculetype == 'v':
        df = pd.read_csv(f'H_Rindler{d}d_v_clean.csv', names=[
    ...
    if moleculetype == 'lambda':
        ...
        plt.legend()
        plt.savefig(
            fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\LinksEntropyRindler_{moleculetype}_{d}d.png', dpi=300, bbox_inches='tight', transparent=True)
        plt.show()
    ...
    plt.xlabel(r'$A/{\ell^{d-2}}$')
    if moleculetype == 'v':
        plt.ylabel(r'$\langle H_V\rangle$')
    ...
    plt.legend()
    plt.savefig(
        fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\BoltzEntropyRindler_{moleculetype}_{d}d.png', dpi=300, bbox_inches='tight', transparent=True)
    plt.show()
  # %%
...
for d in d_array:
    ...
    plt.hist(bList, bins=binsCount[int(d-2)], density=True, histtype='stepfilled',
             color=colors[int(d-2)], label=f'{d-1}+1 Data', alpha=0.5)
...
plt.savefig(
    fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\bepislonDistribution_Rindler_{moleculetype}.png', dpi=300, bbox_inches='tight', transparent=True)
plt.show()
