...
class CausalSet(object):
    ...
    def visualisation(self):
        ...
        if self.dimension == 2:
            ...
            plt.scatter(coordinates[:, 1], coordinates[:, 0], s=90, c='black')
        ...
        plt.savefig(f'Plots/Rindler{seed}.png', dpi = 300, bbox_inches='tight', transparent = True)
        plt.show()
...
if __name__ == "__main__":
    ...
    for seed in seeds:
       ...
       # boundsArray[0][1] = 0.5 #no normalisation
        ...
        c = CausalSet(sprinkling_density=200,    # 0.1-1 for Dynamic Uniform, 1k - 10k for Dynamic Tube, 1k - 10k for Rindler, Empty
                      dimension=2,
                      BHtype='Rindler',           # 'Rindler', 'Dynamic', 'Empty'
                      sprinkling='Tube',     # 'Uniform' or 'Tube' for 'Dynamic'BH 
                      T = 1, 
                      seed = seed)#,                    # T is only needed when BHtype = 'Dynamic'
