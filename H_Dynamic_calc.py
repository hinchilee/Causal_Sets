...
for d in d_array:
    if moleculetype == 'lambda':
        df = pd.read_csv(f'H_Dynamic{d}d_lambda_additional.csv', names=[
                         'rho', 'H', 'b'], header=None)
        df['rho'] = df['rho'].round(1)
    # print(df)
    elif moleculetype == 'v':
        df = pd.read_csv(f'H_Dynamic{d}d_v_additional.csv', names=[
    ...
    for rho in rho_array:
        ...
        print(f'T = {T}')
        if moleculetype == 'lambda':
            totalHarray = [sum(x) for x in itertools.zip_longest(*[[int(x) if x != '' else 0 for x in a.replace(
                '[', '').replace(']', '').split(', ')] for a in df[df['rho'] == rho]['H'].values], fillvalue=0)]
            dataArray = np.array([x for x in itertools.zip_longest(*[[int(x) if x != '' else 0 for x in a.replace(
                '[', '').replace(']', '').split(', ')] for a in df[df['rho'] == rho]['H'].values], fillvalue=0)])
        elif moleculetype == 'v':
        ...
        empiricalavalue = rho**((2-d)/d)*(totalLinks /
                                          iterationsNo) / n_sphere_surfacearea(n=d - 2, r=T)
        ...
        totalLinksErrorList.append(
            np.std(LinksArray)/(totalLinks/iterationsNo))

        if moleculetype == 'lambda':
            # theoryauncorrected
            if d == 4:
                theoryacorrected = 0.173205 + \
                    (-0.0209261)*(d-2)*(rho**(-1/d))/(T*np.sqrt(2))
            elif d == 3:
                theoryacorrected = 0.2188
            elif d == 2:
                theoryacorrected = 2/3
            print(
                f'theoretical a value for rho {d}d after finite rho correction is {theoryacorrected} ')
        ...
        percEntropyError = np.std(MoleculeArray) / \
            ((np.sum(totalHarray) / iterationsNo))
    ...
    if moleculetype == 'lambda':
        ...
        plt.errorbar(AoverlList, totalLinksList,
                     yerr=totalLinksErrorList, capsize=4, linestyle='')
        popt, pcov = curve_fit(linear, AoverlList, totalLinksList,
                               sigma=totalLinksErrorList, absolute_sigma=True)
        ...
        plt.legend()
        plt.savefig(
            fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\LinksEntropyDynamic_{moleculetype}_{d}d.png', dpi=300, bbox_inches='tight', transparent=True)
        plt.show()
    ...
    plt.errorbar(np.array(x), np.array(y_entropyList),
                 yerr=entropyerror, capsize=4, linestyle='')
    plt.xlabel(r'$A/{\ell^{d-2}}$', fontsize=25)
   ...
   # plt.xticks(fontsize = 20)
    ...
    plt.legend()
    plt.savefig(
        fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\BoltzEntropyDynamic_{moleculetype}_d{d}.png', dpi=300, bbox_inches='tight', transparent=True)
    plt.show()
...
for d in d_array:
    if moleculetype == 'lambda':
        df = pd.read_csv(f'H_Dynamic{d}d_lambda_clean.csv', names=[
                         'rho', 'H', 'b'], header=None)
        df['rho'] = df['rho'].round(1)
    # print(df)
    elif moleculetype == 'v':
        df = pd.read_csv(f'H_Dynamic{d}d_v_clean.csv', names=[
                         'rho', 'H',  'subgraphs', 'connected', 'b'], header=None)
        df['rho'] = df['rho'].round(1)
    #   Analyse b
...
plt.legend()
plt.savefig(
    fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\bepislonDistribution_Dynamic_{moleculetype}.png', dpi=300, bbox_inches='tight', transparent=True)
plt.show()
...
plt.errorbar(rho_array, empiricala, yerr=empiricalaerror,
             capsize=4, linestyle='')
...
plt.savefig(
    fr'C:\Users\leehi\OneDrive\Documents\Imperial_tings\Fourth_Year\MSci Project\Thesis\Plots\Fittedvalue_a4_{d}D_Dynamic.png', dpi=300, bbox_inches='tight', transparent=True)
plt.show()
